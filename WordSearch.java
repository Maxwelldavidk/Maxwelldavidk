...
public class WordSearch<T> {
    ...
    public int countWords(){
        int count = 0;
        for (Word<T> word : seen.getKeys()){ //iterate over the words in the seen dictionary
            if(seen.find(word)){
                count ++; // if the word is found increment count
            }
        }
        return count;
    }
    ...
    public List<Word<T>> getWords(){
        ...
        for (Word<T> word : seen.getKeys()){// iterate over the seen dictionary
            if(seen.find(word)){
                words.add(word); // if the word is found add it to the list
            }
        }
        return words;
    }
