...
public class Word<T> {
    ...
    public int hashCode(){
        ...
        int hash = 3; // hash value
        for(int i = 0; i < this.slice.length; i++){ //iterate over each element in the slice
            T element = this.slice[i];

            int elementToHash;
            if(element == null){
                elementToHash = 0;
            } else {
                elementToHash = element.hashCode(); // hash code of the element
        }
           hash = hash * 37 + elementToHash; // update the hash value
        }
        return hash;
    }
